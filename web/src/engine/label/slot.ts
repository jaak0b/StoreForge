 ...
 * face, in plan: the same figure as SLOT_REACH_DEPTH without the end stop (a
 * fused bin has none), so the plate's 45-degree back chamfer, ending one plate
 * thickness behind the channel's back edge, is the widest reach.
 */
export const FUSED_SHELF_REACH_DEPTH = SLOT_FRONT_INSET + SLOT_DEPTH + FUSED_SHELF_THICKNESS;
 ...
 * channel. Unioned with the bin body.
 */
export function buildFusedShelf(m: ManifoldToplevel, params: BinParams): Manifold {
  ...
  return assembleShelf(
    m,
    params,
    [buildShelfStructure(m, params, bodyTop, yBack, FUSED_SHELF_THICKNESS)],
    'Fused label shelf',
  );
}
